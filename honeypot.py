...
if __name__ == "__main__":
    ...
    # use_reloader=False so logs show in main console
    try:
        from waitress import serve
        print("[INFO] Starting production WSGI server (Waitress)...")
        app.debug = True; app.run(host="0.0.0.0", port=8001, debug=True, use_reloader=False)
    except ImportError:
        print("[WARNING] Waitress not installed. Falling back to dev server.")
        app.debug = True; app.run(host="0.0.0.0", port=8001, debug=True, use_reloader=False)
